  ...
  var chartData = {
    ...
    encoding: {
      ...
      "tooltip": [
        ...
        { field: "totalProfitOrLoss", type: "quantitative", title: "totalProfitOrLoss" },
        { field: "sellingDate", type: "ordinal", title: "sellingDate" }
      ],
    }
  };

  const data = await fetch("/result/BNB-USD.json")
    .then(res => res.json())
    .then(res => {
      return res.map(({ buyingDay, totalProfitOrLoss, sellingDay }) =>
        new Object({ buyingDate: buyingDay.Date, totalProfitOrLoss, sellingDate: sellingDay.Date }));
    });
...
const drawTotalProfitOverTime = async () => {
  ...
  const data = [];
  await fetch("/result/BNB-USD.json")
    .then(res => res.json())
    .then(res => {
