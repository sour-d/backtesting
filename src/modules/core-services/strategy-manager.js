...
class LiveStrategyManager {
  ...
  async addStrategy(strategy, isNew = true) {
    ...
    try {
      ...
      if (isNew && isProd) {
        log.info("Creating new strategy in database", { strategyName, stockName, strategyID: strategy.id });
        try {
          await this._retryDbOperation(
            () => createStrategy({
              id: strategy.id, // Use the UUID already generated by the strategy
              strategyName,
              stockName,
              timeFrame,
              config,
          ...
          log.info("Creating new strategy in database successful", { strategyName, stockName, strategyID: strategy.id });
        } catch (error) {
          log.info("Creating new strategy in database failed", { strategyName, stockName, strategyID: strategy.id });
          throw new Error(`Failed to persist strategy: ${error.message}`);
        }
      } else if (!isNew) {
        log.info("Adding existing strategy from database:", { strategyName, stockName, strategyID: strategy.id });
      }
